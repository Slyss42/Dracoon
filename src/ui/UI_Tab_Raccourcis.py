...
from core.shortcuts import _unhook_all, CtrlShiftManager
...
class TabRaccourcisMixin:
    ...
    def _shortcut_action_worker(self):
        """Thread dédié qui exécute les actions de focus sans bloquer keyboard."""
        while True:
            action_fn = self._shortcut_queue.get()
            try:
                action_fn()
            except Exception:
                self._logger.exception("Erreur dans le worker raccourci")

    def _safe_hotkey(self, fn, name: str):
        def wrapper():
            self._shortcut_queue.put(fn)
        return wrapper
    ...
    def _apply_shortcuts(self, silent: bool = False):
        ...
        try:
            ...
            if self._shortcut_next:
                keyboard.add_hotkey(self._shortcut_next, self._safe_hotkey(self._focus_next, "next"), suppress=True)
            if self._shortcut_prev:
                keyboard.add_hotkey(self._shortcut_prev, self._safe_hotkey(self._focus_prev, "prev"), suppress=True)
            if self._shortcut_back:
                keyboard.add_hotkey(self._shortcut_back, self._safe_hotkey(self._focus_back, "back"), suppress=True)
            if self._shortcut_main:
                keyboard.add_hotkey(self._shortcut_main, self._safe_hotkey(self._focus_main, "main"), suppress=True)
            if self._shortcut_ctrl_shift:
                keyboard.add_hotkey(self._shortcut_ctrl_shift, self._safe_hotkey(self._toggle_ctrl_shift, "ctrl_shift"), suppress=True)
    ...
    def _toggle_ctrl_shift(self):
        from core.windows import is_dofus_foreground
        self._ctrl_shift_manager.toggle(is_dofus_foreground)

    def _focus_main(self):
        from core.windows import is_dofus_foreground, focus_dofus_window
    ...
    def _focus_back(self):
        from core.windows import is_dofus_foreground, focus_window
        if not is_dofus_foreground():
            return
    ...
    def _cycle(self, direction: int):
        ...
        from core.windows import focus_window
