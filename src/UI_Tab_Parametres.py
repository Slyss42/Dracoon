...
from PyQt6.QtCore import Qt, QEvent, QObject, QMetaObject, pyqtSlot, QTimer, QPropertyAnimation, QEasingCurve
from PyQt6.QtGui import QFont, QCursor, QKeySequence, QKeyEvent

from logic import _unhook_all, KEYBOARD_OK, WIN32_OK, MoveModeManager, DradidasManager, t
...
class TabParametresMixin:
    ...
    def _build_tab_parametres(self):
        ...
        _param_row(
            ...
            on_change=lambda enabled: __import__("logic").apply_shorten_titles(enabled),
        )
        ...
        sep_layout.setContentsMargins(16, 8, 16, 8)
        sep_line = QFrame()
        sep_line.setFrameShape(QFrame.Shape.HLine)
        sep_line.setFixedHeight(1)
        sep_line.setStyleSheet(f"background-color: {self.GRAY}; border: none;")
        sep_layout.addWidget(sep_line)
        self._modes_layout.insertWidget(self._modes_layout.count() - 1, sep_container)

        # --- En-tête section Modes ---
        #modes_top = QWidget()
        #modes_top.setStyleSheet("background: transparent;")
        #modes_top_layout = QVBoxLayout(modes_top)
        #modes_top_layout.setContentsMargins(16, 4, 16, 8)
        #modes_top_layout.setSpacing(2)
        #lbl_modes_titre = QLabel("Modes")
        #lbl_modes_titre.setFont(self.S.EnTete.font)
        #lbl_modes_titre.setStyleSheet(f"color: {self.TEXT};")
        #modes_top_layout.addWidget(lbl_modes_titre)
        #lbl_modes_sub = QLabel("Activez et configurez les modes de jeu")
        #lbl_modes_sub.setFont(self.S.Info.font)
        #lbl_modes_sub.setStyleSheet(f"color: {self.GRAY};")
        #modes_top_layout.addWidget(lbl_modes_sub)
        #layout.addWidget(modes_top)

        
        # Initialisation manager + construction carte
        ...
        self._move_manager = MoveModeManager(
            ...
            is_dofus_fg_fn  = __import__("logic").is_dofus_foreground,
    ...
    def _build_mode_card(
            ...
            idx = self._modes_layout.count() - 1
            self._modes_layout.insertWidget(idx, root)
            return root
    ...
    def _build_mode_deplacement(self):
        ...
        def _build_content(panel_layout: QVBoxLayout):
            ...
            self._move_shortcut_entry_mode = self._mode_shortcut_entry(
                panel_layout, t("tab.parametres.mode.move.raccourci"), self._shortcut_move, "move"
            )
            # Overlay (label à gauche, checkbox à droite)
            self._mode_toggle_row(
                panel_layout, t("tab.parametres.mode.move.overlay"), self.move_overlay_var
            )
        ...
        self._build_mode_card(
            mode_id          = "deplacement",
            icon             = "🏃",
            title            = t("tab.parametres.mode.move.title"),
            subtitle         = t("tab.parametres.mode.move.description"),
    ...
    def _build_mode_dradidas(self):
        ...
        def _build_content(panel_layout: QVBoxLayout):
            ...
            self._dradidas_shortcut_entry_mode = self._mode_shortcut_entry(
                panel_layout, "Raccourci", self._shortcut_dradidas, "dradidas"
            )

            # Nombre de tours
            self._dradidas_turns_spinbox = self._mode_spinbox_row(
                panel_layout,
                label     = t("tab.parametres.mode.dradidas.tours"),
                min_val   = 2,
                max_val   = 4,
            ...
            # En-tête
            lbl_sadidas = QLabel(t("tab.parametres.mode.dradidas.personnages"))
        ...
        self._build_mode_card(
            mode_id          = "dradidas",
            icon             = "🌿",
            title            = t("tab.parametres.mode.dradidas.title"),
            subtitle         = t("tab.parametres.mode.dradidas.description"),
    ...
    def _trigger_dradidas(self):
        ...
        """
        from logic import is_dofus_foreground, extract_pseudo_from_title
    ...
    def _rebuild_dradidas_char_list(self):
        ...
        if not self._char_order:
            lbl = QLabel(t("tab.parametres.mode.dradidas.notfound"))
            lbl.setFont(self.S.Info.font)
            lbl.setStyleSheet(f"color: {self.GRAY}; background: transparent;")
    ...
    def _reset_config(self):
        ...
        try:
            ...
            with key:
                for name in _build_config(None, None, None).keys():
                    try:
